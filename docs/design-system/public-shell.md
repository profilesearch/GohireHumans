...
2. Primary buyer CTA remains `Request QA` / `Starter QA Offers`.
3. Worker route must be labeled `Open Jobs for Workers`, not generic `Open Jobs`.
4. Footer text and links must use readable text tokens, not low-contrast inline rgba values.
5. Navigation pages should include `style.css?v=20260526-nav-consistency`.
