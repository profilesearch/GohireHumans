...
test.describe('GoHireHumans public/browser regression suite', () => {
  ...
  test('admin refund reports a full refund when a charged hire never started', async ({ page }) => {
    await page.addInitScript(() => {
      sessionStorage.setItem('ghh_token', 'admin-token');
      localStorage.setItem('ghh_user', JSON.stringify({ id: 9, name: 'Admin', is_admin: true }));
    });
    await page.route('https://gohirehumans-production.up.railway.app/**', async route => {
      const url = new URL(route.request().url());
      if (url.pathname === '/admin/orders') return route.fulfill({status:200,contentType:'application/json',body:JSON.stringify({orders:[{id:89,status:'disputed',total_amount:25,worker_name:'Worker',employer_name:'Employer'}]})});
      if (url.pathname === '/admin/resolve-dispute') return route.fulfill({status:200,contentType:'application/json',body:'{"ok":true,"resolution":"refund_to_employer","status":"succeeded","idempotent_replay":false,"refund_scope":"full_charge","amount_cents":2600}'});
      return route.fulfill({status:200,contentType:'application/json',body:'{}'});
    });
    await page.goto('/#/admin/disputes', { waitUntil: 'domcontentloaded' });
    await expect(page.getByText('If a job hire was charged but never started, the full charge including fees is refunded.')).toBeVisible();
    await page.getByRole('button',{name:'Issue refund'}).first().click();
    await page.locator('#admin-password-confirm').fill('step-up-password');
    await page.locator('.modal-overlay').getByRole('button',{name:'Issue refund'}).click();
